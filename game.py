...
class Game:
	...
	def end_screen(self,win): # displays game end message
		global running
		print()
		if (win):
	...
	def show_description(self):
		'''
		(0,2) and (-1,1) are special rooms, with unique different
		behaviour, hence have to be hard coded
		'''
		if (self.player.location == (0,2)): 
			...
			return
		elif (self.player.location in self.battle_pos): # if there is a monster at that location
	...
	def interact(self):
		'''
		Handles all interactable rooms in the game
		'''
		print()
		if (self.player.location == (0,2)):
